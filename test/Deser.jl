...
@testset verbose = true "De" begin
    @testset "Case №1: Simple deserialization" begin
        ...
        exp_kvs = ODict{Symbol,Int64}(:a => 100, :b => 300)
        exp_obj = Foo(100, 300)
        @test Serde.deser(Foo, exp_kvs) == exp_obj

        exp_kvs = ODict{String,Int64}("a" => 100, "b" => 300)
        exp_obj = Foo(100, 300)
        @test Serde.deser(Foo, exp_kvs) == exp_obj

        exp_kvs = ODict{String,Int64}("a" => 100)
        @test_throws "ParamError: parameter 'b::Int64' was not passed or has the value 'nothing'" Serde.deser(
            Foo,
            exp_kvs,
        )

        exp_kvs = ODict{String,Real}("a" => 100.00001, "b" => 300)
    ...
    @testset "Case №2: Deserialization with type casting" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:a => 100, :b => 300.0001)
        exp_obj = Foo2(100, 300.0001)
        @test Serde.deser(Foo2, exp_kvs) == exp_obj

        exp_kvs = ODict{String,Any}("a" => 100, "b" => "300")
    ...
    @testset "Case №3: Custom name" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:a => 100.00001, :b => "2022-01-01")
    ...
    @testset "Case №4: Default value" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:value => 100.00001)
    ...
    @testset "Case №5: Deserialization with nesting" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:f => 10, :bar => ODict{Symbol,Any}(:x => 10, :y => 1000))
    ...
    @testset "Case №6: Deserialization with nothing field" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:a => 100.00001, :b => 100)
    ...
    @testset "Case №8: Deserialization with nothing" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:might_be_field => 2)
        exp_obj = Foo8(nothing, 2)
        @test Serde.deser(Foo8, exp_kvs) == exp_obj

        exp_kvs = ODict{Symbol,Any}()
    ...
    @testset "Case №9: Deserialization with replace default value" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:limit => 11)
        exp_obj = Foo9(11)
    ...
    @testset "Case №10: Deserialization with fill vector" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:bananas => fill(ODict{Symbol,Any}(:weight => 2), 3))
    ...
    @testset "Case №11: Deserialization with deep nesting" begin
        ...
        exp_kvs = ODict{Symbol,Any}(
            :banana => ODict{Symbol,Any}(:weight => ODict{Symbol,Any}(:grams => 2)),
        )
    ...
    @testset "Case №12: Custom deserialization" begin
        ...
        function Serde.deser(::Type{T}, data::S)::T where {T<:DateTime,S<:AbstractString}
            return unix2datetime(parse(Float64, data) * 0.001)
        end

        exp_kvs = ODict{Symbol,Any}(:distance => "1444", :when => "1671688821937")
    ...
    @testset "Case №13: Deserialization vector" begin
        ...
        exp_kvs = [
            ODict{Symbol,Any}(:a => "100.00001", :b => "2022-01-01"),
            ODict{Symbol,Any}(:a => "100.00001", :b => "2022-01-01"),
            ODict{Symbol,Any}(:a => "100.00001", :b => "2022-01-01"),
        ]
    ...
    @testset "Case №14: Deserialization with replace all name" begin
        ...
        exp_kvs = ODict{String,Any}("b-b" => "25", "q-q" => "24", "p-q" => 23.0, "a-p" => 22.0)
    ...
    @testset "Case №16: Deserialization to vector of struct" begin
        ...
        exp_kvs = ODict{Symbol,Int64}[ODict{Symbol,Int64}(:fuel => 25)]
    ...
    @testset "Case №17: Deserialization to dict" begin
        exp_kvs = ODict{Int64,String}(2 => "2", 3 => "3")
        exp_obj = ODict{String,Int64}("2" => 2, "3" => 3)
        @test Serde.deser(ODict{String,Int64}, exp_kvs) == exp_obj
    end
    ...
    @testset "Case №18: Deserialization type casting" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:a => 2, :b => "2")
        exp_obj = Foo21(2, 2)
    ...
    @testset "Case №19: Deserialization to dict of struct" begin
        ...
        exp_kvs = ODict{String,Any}(
            "ABC" => ODict{String,String}("a" => "100", "b" => "hello"),
            "BBB" => ODict{String,String}("a" => "200", "b" => "hi"),
            "CCC" => ODict{String,Union{String,Int64}}("a" => 300, "b" => "holo"),
        )
        exp_obj = ODict{String,Foo22}(
            "BBB" => Foo22(200, "hi"),
            "CCC" => Foo22(300, "holo"),
            "ABC" => Foo22(100, "hello"),
        )
        @test Serde.deser(ODict{String,Foo22}, exp_kvs) == exp_obj
    end
    ...
    @testset "Case №20: Deserialization with inheritance" begin
        ...
        exp_obj = ODict{String,Foo23}(
            "BBB" => Foo23(200, "hi"),
            "CCC" => Foo23(300, "holo"),
            "ABC" => Foo23(100, "hello"),
        )
        @test Serde.deser(ODict{String,Foo23}, exp_kvs) == exp_obj
    end
    ...
    @testset "Case №22: Deserialization Enum" begin
        ...
        exp_kvs = ODict{String,Any}("n" => "enum", "e" => 1)
        exp_obj = Foo25("enum", running)
        @test Serde.deser(Foo25, exp_kvs) == exp_obj

        exp_kvs = ODict{String,Any}("n" => "enum", "e" => :running)
        exp_obj = Foo25("enum", running)
        @test Serde.deser(Foo25, exp_kvs) == exp_obj
    end
    ...
    @testset "Case №23: Testing ClassType" begin
        ...
        exp_kvs = ODict{Symbol,Any}(:a => 1, :b => ODict(:a => 1))
    ...
    @testset "Case №24: Deserialization NamedTuple" begin
        ...
        exp_kvs = ODict{String,Any}(
            "name" => "NamedTuple",
            "not_tuple" => ODict{String,Any}("a" => 10, "b" => 20),
        )
    ...
    @testset "Case №28: Deserializing an empty string" begin
        ...
        exp_kvs = ODict{String,Union{String,Int64}}("z" => 100, "x" => "")
    ...
    @testset "Case №29: Enum deserializing" begin
        ...
        exp_kvs = ODict{String,Any}("distance" => 100, "fuel" => 150, "side" => :Right)
        exp_obj = Journey(150.0, 100.0, Right)
        @test Serde.deser(Journey, exp_kvs) == exp_obj

        exp_kvs = ODict{String,Any}("distance" => 100, "fuel" => 150, "side" => 1)
        exp_obj = Journey(150.0, 100.0, Right)
        @test Serde.deser(Journey, exp_kvs) == exp_obj
    ...
    @testset "Case №32: Custom deserialization union with nothing" begin
        ...
        exp_kvs = ODict{String,Any}("expire_time" => "2023-02-22T21:33:18.187")
    ...
    @testset "Case №33: Deserialization Vector{T} to Set{T}" begin
        ...
        """
        @test deser_json(Foo39, exp_str).a == OSet([2, 3, 1])
        @test deser_json(Foo39, exp_str).b == [1, 2, 3]
        @test deser_json(Foo39, exp_str).c == OSet(["bbb", "aaaa", "ccc"])
        ...
        exp_kvs = ODict{String,Any}("q" => [2, 2, 2, 9])
        exp_obj = Foo40(OSet([2, 9]))
    ...
    @testset "Case №34: Deserialization AbstractString to AbstractString" begin
        ...
        exp_kvs = ODict{String,SubString}("a" => "text")
        exp_obj = Foo42("text")
    ...
    @testset "Case №35: Deserialization Number to Number" begin
        ...
        exp_kvs = ODict{String,Float64}("a" => 2.1)
    ...
    @testset "Case №37: Deserialization Number to Number" begin
        ...
        @test_throws "WrongType: for 'Message{Nothing}' value '" * r".+" * "{String, Any}()' has wrong type 'payload::" * r".+" * "{String, Any}', must be 'payload::Nothing'" deser_json(
            Message{Nothing},
            exp_str,
        )
    end
    ...
    @testset "Case №38: Deserialization with missing field with Union{Nulltype,AnyType}" begin
        ...
        @test Serde.deser(WithNothing, ODict("y" => 1, "x" => nothing)) == WithNothing(1, nothing)
        @test Serde.deser(WithNothing, ODict("y" => 2, "x" => missing)) == WithNothing(2, nothing)
        ...
        @test Serde.deser(WithMissing, ODict("y" => 3, "x" => nothing)) == WithMissing(3, missing)
        @test Serde.deser(WithMissing, ODict("y" => 4, "x" => missing)) == WithMissing(4, missing)
    end
    ...
    @testset "Case №49: JSON deserialization Set" begin
        ...
        exp_obj = MyType49(OSet(Number[1, 2, 3.0]))
    ...
    @testset "Case №50: Deserialization with ignoring field" begin
        ...
        exp_kvs = ODict("a" => "test", "b" => 2, "c" => 3.14)
